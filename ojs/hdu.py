...
class HduModule(OjModule):
    ...
    result_regex_r = (
        r'<tr (bgcolor=#D7EBFF )?align=center >'
        r'<td height=22px>(\d+)</td><td>([0-9\-: ]+)</td>'
        r'<td>(<a (.*) target=_blank>)?<font color=(#?[0-9a-zA-Z]+)>([0-9a-zA-Z<>()_ ]+)</font>(</a>)?</td>'
        r'<td><a href="/showproblem\.php\?pid={pid}">{pid}</a></td>'
        r'<td>(\d+)MS</td>'
        r'<td>(\d+)K</td>'
        r'<td><a href="/viewcode\.php\?rid=(\d+)"  ?target=_blank>(\d+) B</td>'
        r'<td>([A-Za-z\+#]+)</td>'
        r'<td class=fixedsize><a href="/userstatus\.php\?user={username}">{username}</a></td></tr>')
    ...
    def check_login(self):
        ...
        return html.find('<img alt="Author" src="/images/user.png" border=0 height=18 width=18> {}'
