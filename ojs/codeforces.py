...
class CodeforcesModule(OjModule):
	...
	support_languages = [ 'C', 'C++', 'Python', 'JavaScript', 'C#', 'D', 'Go', 'Haskell', 'Java', 'Kotlin', 'OCaml', 'Perl', 'PHP', 'Ruby', 'Rust', 'Scala', 'Pascal' ]
	...
	def _fetch_detail_process(self, runtime):
		...
		while runtime.judging == True or local_id < runtime.sync_id:
			while local_id >= runtime.sync_id and runtime.judging == True:
				time.sleep(0.1)
	...
	def _check_printf_lld(self, code):
		if code.find('%lld') == -1:
			return
		return_value = sublime.ok_cancel_dialog('Found \"%lld\" in your code.\nAre you sure to submit it anyway?', 'Yes')
		if not return_value:
			raise ExitScript()
