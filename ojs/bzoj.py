...
class BzojModule(OjModule):
	...
	result_regex_r = r"<tr align=center class='(even|odd)row'><td>(\d+)<td><a href='userinfo\.php\?user={name}'>{name}</a><td><a href='problem\.php\?id={pid}'>{pid}</a><td>(<a href='ceinfo\.php\?sid=\d+'>)?<font color=#?[0-9a-zA-Z]+>([a-zA-Z_&]+)</font>(</a>)?<td>((\d+) <font color=red>kb</font>|------)<td>((\d+) <font color=red>ms</font>|------)<td><a target=_blank href=showsource\.php\?id=\d+>[a-zA-Z+]+</a>/<a target=_self href=.submitpage\.php\?id={pid}&sid=\d+.>Edit</a><td>\d+ B<td>[0-9\- :]+</tr>"
	detail_regex_r = r"<tr align=center class='(even|odd)row'><td>(\d+)<td><a href='userinfo\.php\?user={name}'>{name}</a><td><a href='problem\.php\?id={pid}'>{pid}</a><td>(<a href='ceinfo\.php\?sid=\d+'>)?<font color=#?[0-9a-zA-Z]+>([a-zA-Z_&]+)</font>(</a>)?<td>(\d+) <font color=red>kb</font><td>(\d+) <font color=red>ms</font><td><a target=_blank href=showsource\.php\?id=\d+>[a-zA-Z+]+</a>/<a target=_self href=.submitpage\.php\?id={pid}&sid=\d+.>Edit</a><td>\d+ B<td>[0-9\- :]+</tr>"
