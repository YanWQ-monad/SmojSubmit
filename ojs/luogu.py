...
class LuoguModule(OjModule):
    ...
    post_type = 'json'
    support_languages = [
        'C++', 'C', 'Java', 'Pascal', 'Python', 'JavaScript', 'Ruby', 'Go', 'Rust',
        'PHP', 'C#', 'Haskell', 'Kotlin', 'Scala', 'Perl']
    ...
    feinjection_regex = re.compile(
        r'window\._feInjection = JSON.parse\(decodeURIComponent\("([-a-zA-Z0-9()@:%_\+.~#?&\/=]+)"\)\);')
    ...
    def load_result(self, runtime):
        data, resp = self.get(
            self.record_api_url.format(runtime.judge_id),
            merge_dict(self.headers, dict(Referer=self.show_problem_url.format(runtime.pid))))
        data = json.loads(data)
    ...
    def _unlock_with_2FA(self):
        ...
        while True:
            ...
            sublime.active_window().show_input_panel(
                '[{}] Enter 2FA code'.format(PLUGIN_NAME), initial_msg, got_input, None, on_cancal)

            while code is None and cancal is False:
                time.sleep(0.1)
            if cancal:
                raise ExitScript()
